...
var ConfigOptionsInfo = Options{{
	...
	Default: false,
	Help:    "Enable opt-in HTTP domain fronting in shared transport.",
	Groups:  "Networking",
}, {
	Name:    "fronting_target",
	Default: "",
	Help:    "Fronting target hostname used for network dial; original request port is preserved.",
	Groups:  "Networking",
}, {
	Name:    "fronting_domains",
	Default: []string{},
	Help:    "Comma-separated hostname patterns eligible for fronting (exact or *.).",
	Groups:  "Networking",
}, {
	Name:    "fronting_sni",
	Default: "",
	Help:    "Optional TLS SNI override for fronted requests (defaults to fronting target host).",
	Groups:  "Networking",
}}
...
func validateFrontingDomainRules(domains []string) error {
	...
	for _, part := range strings.Split(raw, ",") {
		...
		if rule == "*" || (strings.Contains(rule, "*") && !strings.HasPrefix(rule, "*.")) {
			return fmt.Errorf("--fronting-domains has unsupported wildcard rule %q (use exact host or *.)", part)
		}
